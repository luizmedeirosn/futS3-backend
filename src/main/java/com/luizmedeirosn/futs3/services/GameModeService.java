...
public class GameModeService {
  ...
  public GameModeResponseDTO findById(Long id) {
    var gameModeMin =
        gameModeRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("GameMode ID not found:" + id));
    ...
    return new GameModeResponseDTO(
        gameModeMin.getId(), gameModeMin.getFormationName(), gameModeMin.getDescription(), positions);
  }
  ...
  private List<PositionResponseDTO> mapToPositionResponseDTO(
      Map<Position, List<PositionParametersDataDTO>> positionsParameters) {
    return positionsParameters.entrySet().stream()
        .map(
            entry ->
  ...
  public GameModeResponseDTO update(Long id, GameModeRequestDTO gameModeRequestDTO) {
    try {
      ...
      gameModeRepository.saveAllPositions(gameMode.getId(), gameModeRequestDTO.positions(), entityManager);
